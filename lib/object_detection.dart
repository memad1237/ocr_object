...
class ObjectDetection {
  static const String _modelPath = 'assets/models/detect_card_v2.tflite';
  ...
  Uint8List analyseImage(String imagePath) {
    ...
    // Classes
    final classesRaw = output[3][0] as List<double>;
    final classes = classesRaw.map((value) => value.toInt()).toList();
    log('Classes: $classes');

    // Scores
    final scores = output[0][0] as List<double>;
    log('Scores: $scores');
      ...
      if (scores[i] > 0.6) {
        ...
        for (var i = 0; i < numberOfDetections; i++) {
          if (scores[i] > 0.2) {
            ...
            // Additional bounding boxes (example: creating boxes around detected IDs)
  ...
  List<double> normalizeInputData(List<double> inputData, double inputMean, double inputStd) {
    List<double> normalizedData = [];

    for (int i = 0; i < inputData.length; i++) {
      double normalizedValue = (inputData[i] - inputMean) / inputStd;
  ...
  List<List<Object>> _runInference(
    ...
    final output = {
      ...
      1: [List<List<num>>.filled(2, List<num>.filled(4, 0))],
      2: [0.0],
      3: [List<num>.filled(2, 0)],
    };
